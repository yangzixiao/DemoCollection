...
public class BadgeView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        shapeRectF.left = 0;
        shapeRectF.top = 0;
        shapeRectF.right = getMeasuredWidth();
        shapeRectF.bottom = getMeasuredHeight();
        ...
        switch (currentShape) {
            case SHAPE_CIRCLE:
                ...
                break;
            case SHAPE_OVAL:
                canvas.drawOval(shapeRectF, backgroundPaint);
                canvas.drawText(showText, getMeasuredWidth() / 2f, getMeasuredHeight() / 2f + (textH / 2f - fontMetrics.descent), numberPaint);
                break;
            case SHAPE_RECTANGLE:
                canvas.drawRect(shapeRectF, backgroundPaint);
                canvas.drawText(showText, getMeasuredWidth() / 2f, getMeasuredHeight() / 2f + (textH / 2f - fontMetrics.descent), numberPaint);
                break;
            case SHAPE_SQUARE:
                int sideLength = Math.min(getMeasuredHeight(), getMeasuredWidth());
                canvas.drawRect(0,0,sideLength,sideLength, backgroundPaint);
                canvas.drawText(showText, sideLength / 2f, sideLength / 2f + (textH / 2f - fontMetrics.descent), numberPaint);
                break;
            case SHAPE_ROUND_RECTANGLE:
                canvas.drawRoundRect(shapeRectF, dip2px(getContext(), 5), dip2px(getContext(), 5), backgroundPaint);
                canvas.drawText(showText, getMeasuredWidth() / 2f, getMeasuredHeight() / 2f + (textH / 2f - fontMetrics.descent), numberPaint);
                break;
        }
